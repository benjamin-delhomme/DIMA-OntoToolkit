...
def get_inform_techniques(processed_article: dict) -> dict:
    return {
        # "xxxxxxxxxx": get_inform_technique(processed_article,"texxxx",extract_technique_texxxx),
        # Add more techniques as needed
    }

def get_inform_technique(article: Dict, technique_code: str, extract_fn: Callable[[Dict], List[Dict]]) -> List[Dict]:
    """
    ...
    for motif in article.get("motifs", []):
        ...
        verbose_label = f"Inform - {tech_code} Extraction in {motif_id}"

        cache_result = load_or_compute_cache(
            cache_key=cache_key,
            cache_type="inform_cache",
