...
def get_technique_te0132(article: Dict) -> List[Dict]:
    """
    ...
    for motif in article.get("motifs", []):
        ...
        cache_key = f"article_{article_id}-motif_{motif_id}-TE0132"
        verbose_label = f"Detect - TE0132 Extraction in {motif_id}"

        cache_result = load_or_compute_cache(
            cache_key=cache_key,
            cache_type="detect_cache",
            compute_fn=lambda m=motif: {
                "rows": extract_technique_te0132(m)
            },
